...
export const MEDIUM_CONFIDENCE_PATTERNS: SecretPattern[] = [
    ...
    // Database URLs
    { 
        pattern: /^(mongodb|postgresql|mysql|redis|sqlite):\/\/[^@]+@[^:]+:\d+\/[^?]+/, 
        type: 'database_url', 
    ...
    { 
        pattern: /^-----BEGIN\s+(CERTIFICATE|PRIVATE\s+KEY|RSA\s+PRIVATE\s+KEY)-----/, 
...
export const SECRET_KEY_PATTERNS: SecretKeyPattern[] = [
    ...
    { pattern: /client[_-]?secret/i, description: 'Client secret', confidence: 'high' },
    { pattern: /app[_-]?secret/i, description: 'App secret', confidence: 'high' },
    { pattern: /private[_-]?key/i, description: 'Private key', confidence: 'high' },
    ...
    { pattern: /db[_-]?password/i, description: 'Database password', confidence: 'medium' },
    { pattern: /database[_-]?url/i, description: 'Database URL', confidence: 'medium' },
    { pattern: /connection[_-]?string/i, description: 'Connection string', confidence: 'medium' },
    { pattern: /redis[_-]?password/i, description: 'Redis password', confidence: 'medium' },
    { pattern: /mongodb[_-]?uri/i, description: 'MongoDB URI', confidence: 'medium' },
    { pattern: /postgres[_-]?password/i, description: 'PostgreSQL password', confidence: 'medium' },
    { pattern: /mysql[_-]?password/i, description: 'MySQL password', confidence: 'medium' },
    { pattern: /aws[_-]?secret/i, description: 'AWS secret', confidence: 'medium' },
    { pattern: /azure[_-]?key/i, description: 'Azure key', confidence: 'medium' },
    { pattern: /gcp[_-]?key/i, description: 'Google Cloud key', confidence: 'medium' },
    { pattern: /docker[_-]?password/i, description: 'Docker password', confidence: 'medium' },
    { pattern: /kubernetes[_-]?secret/i, description: 'Kubernetes secret', confidence: 'medium' },
    { pattern: /slack[_-]?webhook/i, description: 'Slack webhook', confidence: 'medium' },
    { pattern: /discord[_-]?webhook/i, description: 'Discord webhook', confidence: 'medium' },
    { pattern: /github[_-]?token/i, description: 'GitHub token', confidence: 'medium' },
    { pattern: /gitlab[_-]?token/i, description: 'GitLab token', confidence: 'medium' },
    { pattern: /stripe[_-]?key/i, description: 'Stripe key', confidence: 'medium' },
    { pattern: /paypal[_-]?key/i, description: 'PayPal key', confidence: 'medium' }
];
...
export function isSecretValue(value: string, minConfidence: 'high' | 'medium' | 'low' = 'low'): boolean {
...
export function isSecretKey(key: string, minConfidence: 'high' | 'medium' | 'low' = 'low'): boolean {
    const confidenceLevels = { high: 3, medium: 2, low: 1 };
    const minLevel = confidenceLevels[minConfidence];
