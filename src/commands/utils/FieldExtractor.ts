...
export class FieldExtractor {
    static extractFieldValue(recordDetails: any, fieldType: KEEPER_NOTATION_FIELD_TYPES, itemName: string): string | null {
        try {
            const fields = fieldType === KEEPER_NOTATION_FIELD_TYPES.CUSTOM_FIELD
                ? recordDetails.custom
                : recordDetails.fields;

            if (!Array.isArray(fields)) {
                logger.logError(`Invalid field structure for ${fieldType}: ${itemName}`);
                return null;
            }

            // Efficient search using find()
            const field = fields.find((f: any) => f.label === itemName || f.type === itemName);
