...
export class RunSecurelyHandler extends BaseCommandHandler {
    ...
    async execute(): Promise<void> {
        ...
        if (!await this.canExecute()) {
            logger.logDebug("RunSecurelyHandler.execute: canExecute returned false, aborting");
            return;
        }
        ...
        try {
            ...
            logger.logDebug(`RunSecurelyHandler: Selected workspace path length: ${workspaceRoot?.length || 0}`);

            logger.logDebug("RunSecurelyHandler: Starting environment file selection");
            const selectedEnvFile = await this.selectEnvironmentFile(workspaceRoot);
            logger.logDebug(`RunSecurelyHandler: Selected environment file path length: ${selectedEnvFile?.length || 0}`);

            logger.logDebug("RunSecurelyHandler: Getting command from user");
            const command = await this.getCommandFromUser();
            logger.logDebug(`RunSecurelyHandler: User provided command length: ${command?.length || 0}`);
            ...
            logger.logDebug(`RunSecurelyHandler: Resolved ${Object.keys(resolvedEnv).length} secrets`);
            ...
            logger.logDebug("RunSecurelyHandler: Terminal created and command started successfully");
    ...
    private async selectWorkspace(): Promise<string> {
        logger.logDebug("RunSecurelyHandler.selectWorkspace: Starting workspace selection");
        const workspaceFolders = workspace.workspaceFolders;
        if (!workspaceFolders || workspaceFolders.length === 0) {
            logger.logDebug("RunSecurelyHandler.selectWorkspace: No workspace folders found");
            throw new Error('Open a folder/workspace first');
        }

        if (workspaceFolders.length === 1) {
            logger.logDebug(`RunSecurelyHandler.selectWorkspace: Single workspace found - name length: ${workspaceFolders[0].name?.length || 0}`);
            return workspaceFolders[0].uri.fsPath;
        }

        logger.logDebug(`RunSecurelyHandler.selectWorkspace: Multiple workspaces found - count: ${workspaceFolders.length}`);
        ...
        if (!selected) {
            logger.logDebug("RunSecurelyHandler.selectWorkspace: User cancelled workspace selection");
    ...
    private async selectEnvironmentFile(workspaceRoot: string): Promise<string> {
        ...
        const envFileNames = envFiles.map(file => path.relative(workspaceRoot, file));
    ...
    private async getCommandFromUser(): Promise<string> {
        ...
        if (!command) {
            throw new Error('No command entered. Please enter a command to run with Keeper secrets injected.');
        }
     ...
     */
    private async resolveSecrets(selectedEnvFile: string): Promise<Record<string, string>> {
    ...
    private async fetchAndResolveSecrets(
        recordGroups: Map<string, Array<{ key: string; fieldType: KEEPER_NOTATION_FIELD_TYPES; itemName: string }>>,
