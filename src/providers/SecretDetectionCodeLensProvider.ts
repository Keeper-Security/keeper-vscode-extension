...
export class SecretDetectionCodeLensProvider implements vscode.CodeLensProvider {
    ...
    public provideCodeLenses(document: vscode.TextDocument, token: vscode.CancellationToken): vscode.ProviderResult<vscode.CodeLens[]> {
        ...
        try {
            ...
            if (codeLenses.length > 0) {
                logger.logDebug(`Created ${codeLenses.length} CodeLens items for ${document.fileName}`);
            }
        } catch (error) {
    ...
    private createSecretCodeLens(match: ParserMatch, document: vscode.TextDocument): vscode.CodeLens | null {
        try {
